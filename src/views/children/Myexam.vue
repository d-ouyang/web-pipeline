...
export default {
  ...
  methods: {
    ...
    bindBtn(item) {
      console.log(item)
      if (item.timing == 2) { //  下载准考证
        console.log(`${this.IMG_BASE_URL}${item.registration}`)
        window.open(`${this.IMG_BASE_URL}${item.registration}`)
      } else if (item.timing == 5) { // 重新上传文件
        console.log('重新上传文件')
        let params = {
    ...
    _handleArr (arr) {
      ...
      for (let i in arr) {
        ...
        // let examDateEndTime = new Date(arr[i].endDate).getTime()
